 ...
 * made that action
 */
var AIAction = function(pos, playerToken) {
    ...
    this.applyTo = function(state) {
        ...
        var aiToken = playerToken === "X" ? "O" : "X";
        //put the letter on the board
        next.board[this.movePosition] = state.turn;
        if(state.turn === aiToken)
            next.oMovesCount++;
 ...
 */
var AI = function(level, playerToken) {
    ...
    function minimaxValue(state) {
        if(state.isTerminal()) {
            ...
            return Game.score(state, playerToken);
        }
        else {
            var stateScore; // this stores the minimax value we'll compute

            if(state.turn === playerToken)
            ...
            var availableNextStates = availablePositions.map(function(pos) {
                var action = new AIAction(pos, playerToken);
    ...
    function takeABlindMove(turn) {
        ...
        var action = new AIAction(randomCell, playerToken);
    ...
    function takeANoviceMove(turn) {
        ...
        var availableActions = available.map(function(pos) {
            var action =  new AIAction(pos, playerToken); //create the action object
